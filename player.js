...
function playerAddEventListener() {
    try {
...
function playerRemoveEventListener() {
    try {
...
function eventStateChange(e) {
    try {
        switch (e.data) {
            case YOUTUBE_STATE.UNSTARTED:
                postCommand(YOUTUBE_COMMAND.FROM_IFRAME.UPDATE_STATE, { state: YOUTUBE_STATE.UNSTARTED });
                break;
            case YOUTUBE_STATE.ENDED:
                try {
                    // Handle player
                    player.pauseVideo();
                    ...
                    // Post command UPDATE_STATE
                    postCommand(YOUTUBE_COMMAND.FROM_IFRAME.UPDATE_STATE, { state: YOUTUBE_STATE.ENDED });
                } catch (err) {}
                break;
            case YOUTUBE_STATE.PLAYING:
                isPaused = false;
                postCommand(YOUTUBE_COMMAND.FROM_IFRAME.UPDATE_STATE, { state: YOUTUBE_STATE.PLAYING });
                break;
            case YOUTUBE_STATE.PAUSED:
                isPaused = true;
                postCommand(YOUTUBE_COMMAND.FROM_IFRAME.UPDATE_STATE, { state: YOUTUBE_STATE.PAUSED });
                break;
            case YOUTUBE_STATE.BUFFERING:
                postCommand(YOUTUBE_COMMAND.FROM_IFRAME.UPDATE_STATE, { state: YOUTUBE_STATE.BUFFERING });
                break;
        }
    } catch (err) {}
};
